...
class Database
{
	...
	static function connect($config)
	{
		if ($config["server"] != "mysql") {
			die("Class Database(connect): Server has to be mysql");
		}
...
class MuscariSocket
{
	static function listen($messageHandler)
	{
		...
		if (!is_numeric($socket_id)) {
			die("Class MuscariSocket(Listen): Save socket to database or getting ID failed.");
		}

		# Create socket
		$LISTEN_SOCKET = socket_create(AF_UNIX, SOCK_STREAM, 0);
		if (
			socket_bind($LISTEN_SOCKET, __DIR__ . "/../sockets/" . $socket_id . ".sock") and
